...
import canva from 'html2canvas';
import { Horario } from '../../Data/groupManager.js';

function exportPDF(tableID: string, html2canvas: typeof canva) {
...
async function exportCombinationsPDF(combinations: Horario[][], html2canvas: typeof canva, setPage: (numDeCombinacion: number) => Promise<void>) {
  ...
  for (let numDeCombinacion = 0; numDeCombinacion < combinations.length; numDeCombinacion++) {
    ...
    const table = await waitForElm("#exportScheduleClassTable" + numDeCombinacion) as HTMLElement;

    if(!table) {
      console.error("No se encontró el elemento con id " + "#exportScheduleClassTable" + numDeCombinacion);
      return;
    }
